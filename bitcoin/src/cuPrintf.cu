...
#define CUPRINTF_PREAMBLE                      \
  ...
  if ((start = getNextPrintfBufPtr()) == NULL) \
    return 0;                                  \
...
extern "C" cudaError_t cudaPrintfDisplay(void *outputFP, bool showThreadID)
{
  ...
  else if (magic == CUPRINTF_SM11_MAGIC)
  {
    ...
    if (sync_printfs)
      doPrintfDisplay(
          showThreadID, clearOnPrint, printfbuf_device, printfbuf_device + printfbuf_len, endptr, printfbuf_device + printfbuf_len);
