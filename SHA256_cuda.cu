...
__global__ void sha256_kernel(const uint8_t *data, uint32_t *output, size_t len)
{
  ...
  for (int i = 0; i < 16; ++i)
  {
    w[i] = (data[idx * 64 + i * 4] << 24) | (data[idx * 64 + i * 4 + 1] << 16) | (data[idx * 64 + i * 4 + 2] << 8) |
           (data[idx * 64 + i * 4 + 3]);
  }
...
void benchmark(const string &input, int num_hashes)
{
  ...
  vector<uint8_t> h_data(num_hashes * 64);
  vector<uint32_t> h_output(num_hashes * 8);

  for (size_t i = 0; i < num_hashes; ++i)
  {
    memcpy(h_data.data() + i * 64, input.data(), len);
    h_data[i * 64 + len] = 0x80;  // Padding
    if (len < 56)
    {
      uint64_t bit_len = len * 8;
      h_data[i * 64 + 56] = (bit_len >> 56) & 0xff;
      h_data[i * 64 + 57] = (bit_len >> 48) & 0xff;
      h_data[i * 64 + 58] = (bit_len >> 40) & 0xff;
      h_data[i * 64 + 59] = (bit_len >> 32) & 0xff;
      h_data[i * 64 + 60] = (bit_len >> 24) & 0xff;
      h_data[i * 64 + 61] = (bit_len >> 16) & 0xff;
      h_data[i * 64 + 62] = (bit_len >> 8) & 0xff;
      h_data[i * 64 + 63] = bit_len & 0xff;
    }
  }
  cout << "[Allocating Memory]" << endl;
  uint8_t *d_data;
  uint32_t *d_output;
  cudaMalloc(&d_data, num_hashes * 64);
  cudaMalloc(&d_output, num_hashes * 8 * sizeof(uint32_t));

  cudaMemcpy(d_data, h_data.data(), num_hashes * 64, cudaMemcpyHostToDevice);

  int threadsPerBlock = 256;
  int blocksPerGrid = (num_hashes + threadsPerBlock - 1) / threadsPerBlock;
  auto start = chrono::high_resolution_clock::now();
  cout << "[Start Calculating]" << endl;
  sha256_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_data, d_output, num_hashes);
